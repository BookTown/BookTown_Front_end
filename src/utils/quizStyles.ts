export const getOptionStyle = (status: 'correct' | 'wrong' | 'default'): string => {
  switch (status) {
    ...
    case "wrong":
      return "bg-white border-[1.5px] border-[#C75C5C] text-black";
    default:
...
export const getStatusBadge = (status: 'correct' | 'wrong' | 'default') => {
  ...
  return {
    className: status === 'correct'
      ? 'absolute -top-2 right-2 px-3 py-1 text-lg rounded-full shadow-sm bg-[#4B8E96] text-white border border-black/20'
      : 'absolute -top-2 right-2 px-3 py-1 text-lg rounded-full shadow-sm bg-[#C75C5C] text-white border border-black/20',
